...
function displayJobs(jobs) {
    ...
    if (jobs != null) {
      ...
      $('#AddJobButton').hide();
        ...
        console.log(typeof job.Start_Date)
          let row = `<tr>
                  <td>${job.JobId}</td>
                  <td>${job.JobName}</td>
                  ...
                  row += `<td>
                    ...
                    <span class="oi oi-trash" data-toggle="tooltip" title="Delete Job"></span></button>
                  </td>`          
...
function displayCategories(categories) {
  ...
  const catLinks = categories.map(category => {
    ...
    return `<a href="#" class="list-group-item list-group-item-action" onclick="updateJobsView(${category.CategoryId})">${category.CategoryName}</a>`;
  });

  // use  unshift to add a 'Show all' link at the start of the array of catLinks
  catLinks.unshift(`<a href="#" class="list-group-item list-group-item-action" onclick="loadJobs()">Show all</a>`);
...
function getJobForm() {
  ...
  const pStart_Date = document.getElementById('Start_Date').value;

  const pFinish_Date = document.getElementById('Finish_Date').value;
...
async function addOrUpdateJob() {
  ...
  async function prepareJobUpdate(id) {
    try {
        ...
        document.getElementById('Finish_Date').value = job.Finish_Date;
