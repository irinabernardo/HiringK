...
let validateId = (id) => {
    ...
    if (validator.isNumeric(id + '', { no_symbols: true, allow_negatives: false })) {
        ...
        console.log("Job validator: invalid id parameter");
    }
    // validation failed
    return false;
}
...
let validateNewJob = (formJob) => {
    ...
    if (
        ...
        !validator.isEmpty(formJob.Start_Date) &&
        !validator.isEmpty(formJob.Finish_Date) &&

        !validator.isEmpty(formJob.UserEmail) &&
        !validator.isEmpty(formJob.Lastupdate)) 
    {
        // Validation passed
        ...
        validatedJob = new Job(
                ...
                validator.escape(formJob.UserEmail),
                validator.escape(formJob.Lastupdate)
...
let validateUpdateJob = (formJob) => {
    ...
    // debug to console - if no data
    if (formJob === null) {
        console.log("validateUpdateJob(): Parameter is null");
    }
    ...
    // appending + '' to numbers as the validator only works with strings
    if (
        validator.isNumeric(formJob.JobId + '', { no_symbols: true, allow_negatives: false }) &&
        validator.isNumeric(formJob.CategoryId + '', { no_symbols: true, allow_negatives: false }) && 
        !validator.isEmpty(formJob.JobName) && 
        !validator.isEmpty(formJob.JobDescription) &&
        !validator.isEmpty(formJob.Start_Date) &&
        ...
        
{
        ...
        // debug
        console.log("validateUpdateJob(): Validation failed");
    }
